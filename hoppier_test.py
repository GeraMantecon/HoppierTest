...
def to_SnackList(snack_string):
    snacks = [snack.strip() for snack in snack_string.replace(' and ',',').split(',')]
    return snacks
...
def main():
    ...
    products = dict([(product['title'],product['variants'][0]['price']) for product in productsData['products']])
    ...
    for snack in list(set(list(zip(*snackerSnack))[1])):
        print(snack)
    #B)
    print('\n'+'What\'re the emails of the snackers who listed those as a \'fave_snack\'?')
    for email in list(zip(*snackerSnack))[0]:
        print(email)
    #C)
